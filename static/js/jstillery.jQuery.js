  ...
  var JStillery = function(element, options) {
    ...
    this._initEditorPanel = function(element) {
      ...
      var panel_menu = $(
        ...
        + '  <div class="btn-group">'
        + '    <button class="btn-action-beautify btn btn-default"><i class="fa fa-fw fa-magic"></i> 美化</button>'
        + '    <button class="btn-action-deobfuscate btn btn-default"><i class="fa fa-fw fa-eye-slash"></i> 反混淆</button>'
        //+ '    <button class="btn-action-deobfuscate-jser btn btn-default"><i class="fa fa-fw fa-eye-slash"></i> Deobfuscate With JSer</button>'
        + '  </div>'
        + '  <div class="btn-group">'
        + '    <button style="pointer-events:all" class="btn-action-dynamicpoc btn btn-default disabled" data-toggle="tooltip" data-placement="bottom" title="生成用于动态分析的POC">&nbsp;<i class="fa fa-fw fa-gamepad"></i>&nbsp;</button>'
        + '  </div>'
    ...
    this.bindBeautify = function() {
      ...
      binded_button.on('click', data, function(event) {
        try {
          ...
          event.data.jstillery.log("代码美化(格式化)完成。Code beautified.", 1);
        } catch (e) {
          event.data.jstillery.log("美化(格式化)错误。Beautify Error: " + e.index + " " + e.description, 0);
        }
      })
    }
    var use_Remote = true;
      ...
      if (use_Remote) {
        ...
        $.ajax({
          ...
          function(a) { 
            ...
            if (a.error)
              add_notification("JavaScript错误。JavaScript Error: " + a.error.description + "<br>行 Line: " + a.error.lineNumber
                + ", Column:" + a.error.column, "error");
            // else{
            //  add_notification ("Code deobfuscated", "info");
            // }
          }).fail(function (argument) {
            console.log("回退到本地 Fallback to local")
    ...
    this.bindDeobfuscate = function(targetEditor) {
      ...
      binded_button.on('click', data, function(event) {
        ...
        try {
          ...
          console.log(e);
          event.data.jstillery.log("反混淆错误。Deobfuscation Error: " + e.index + " " + e.description, 0);
        }
      });
    }
    ...
    this.log("面板初始化完成。 Panel Initialized, id: " + settings.id);
